...
from hcfcd_tools import TOOL_GetCompressionStatus
from hcfcd_tools import TOOL_CompressFGDB
from hcfcd_tools import TOOL_DecompressFGDB

from hcfcd_constants.values import PORTAL_URL
...
class CheckCompressionStatus:
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Get Compression Status (Console Message)"
        self.description = ""
        self.category = "File Geodatabase Compression"
    ...
    def getParameterInfo(self):
        ...
        item_type = arcpy.Parameter(
            ...
            direction="Input")
        item_type.filter.type = "ValueList"
        item_type.filter.list = ["File Geodatabase", "Feature Dataset", "Feature Class", "Table"]

        fgdbs = arcpy.Parameter(
            displayName="File Geodatabases",
            name="fgdbs",
            datatype="DEWorkspace",
            parameterType="Optional",
            direction="Input",
        ...
        fgdbs.filter.list = ["Local Database"]
        ...
        params = [item_type, fgdbs, featuredataset, featureclass, table]
        return params
    ...
    def updateParameters(self, parameters):
        ...
        fgdbs = parameters[1]
        featuredataset = parameters[2]
        featureclass = parameters[3]
        table = parameters[4]

        if item_type.altered and not item_type.hasBeenValidated:
            check_list = [fgdbs, featuredataset, featureclass, table]
            [setattr(i, "enabled", False) for i in check_list]
            [setattr(i, "value", None) for i in check_list]
            if item_type.valueAsText == "File Geodatabase":
                fgdbs.enabled = True
            elif item_type.valueAsText == "Feature Dataset":
                featuredataset.enabled = True
            elif item_type.valueAsText == "Feature Class":
                featureclass.enabled = True
            elif item_type.valueAsText == "Table":
                table.enabled = True
            else:
                pass
    ...
    def execute(self, parameters, messages):
        ...
        if __name__ == "__main__":
            item_type = parameters[0].valueAsText

            input_dict = {
                "File Geodatabase":parameters[1].valueAsText,
                "Feature Dataset":parameters[2].valueAsText,
                "Feature Class":parameters[3].valueAsText,
                "Table":parameters[4].valueAsText
                }
            
            items = input_dict[item_type].split(";")
            cleaned_items = [i.replace("'", "") for i in items]

            arcpy.AddMessage(cleaned_items)
            TOOL_GetCompressionStatus.main(items=cleaned_items)
...
class CompressionFGDB:
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Compress FGDB Data"
        self.description = ""
        self.category = "File Geodatabase Compression"
    ...
    def isLicensed(self):
        """Set whether the tool is licensed to execute."""
        if os.getlogin().upper() not in ['TEDSMITH', 'NAMCDONALD', "ELSMITH", 'RCOMADURAN']:
            return False
        return True
    ...
    def execute(self, parameters, messages):
        ...
        if __name__ == "__main__":
            ...
            arcpy.AddMessage(cleaned_items)
            TOOL_CompressFGDB.main(item_type=item_type, items=cleaned_items)
...
class DecompressionFGDB:
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Decompress FGDB Data"
        self.description = ""
        self.category = "File Geodatabase Compression"
    ...
    def execute(self, parameters, messages):
        ...
        arcpy.AddMessage(cleaned_items)
        TOOL_DecompressFGDB.main(item_type=item_type, items=cleaned_items)
